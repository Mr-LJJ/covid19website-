...
<!--比赛项目自2020.05.12开坑 2020.05.19完成-所有代码前端后端CSS均为自己手打|希望无BUG-->
<!--引用Charts.js https://github.com/chartjs/Chart.js-->
<!--引用Sweetalert2 https://github.com/sweetalert2/sweetalert2-->
<!--本代码遵循MIT协议https://github.com/Mr-LJJ/covid19website--->
<?php //php开始
error_reporting(E_ALL^E_WARNING);//关闭一些玄学事情发生的两条指令(关闭PHP提示和警告)
error_reporting(E_ALL^E_NOTICE);
include("./analysis.php");//引入隔壁PHP
...
if(mysqli_connect_errno($conn)){//如果数据库连不上就终止程序提示
  die("数据库BOOM".mysqli_connect_error());
}
mysqli_query($conn,"set names utf8");//设置字符库为utf-8
$sql ="SELECT * FROM ncovdata ORDER BY id DESC LIMIT 1";//提取数据库最后一条记录(提取数据库最后的ID和时间)
$query=mysqli_query($conn,$sql);
while ($row = mysqli_fetch_array($query)) {
  $yesid=$row['id'];
  $mysqltime=$row['time'];
}
$id = $yesid + '1';//ID自增1用于写入数据库的ID
$sql2 ="SELECT * FROM ncovdata WHERE DATEDIFF(time,NOW())=-1";//提取数据库昨日最后的记录,用作页面较昨日对比
...
$sql3="SELECT * FROM (select * from ncovdata order by id desc limit 5) aa ORDER BY id";//提取最后5条数据，用作折线图。(发生玄学事情，只能笨比方法解决
...
<html lang="zh" dir="ltr">
  ...
  <body>
      ...
      <div class="data_china">
        <div class="totalConfirmed">
          ...
          <div class="number" id="number" style="color:<?php if ($totalConfirmed > '50000'){echo 'RGB(163,29,19)';}//判断数字大小颜色 ?>;">
            <?php echo $totalConfirmed; ?></div><p class="added" style="display: block;">较昨日<span style="color:RGB(163,29,19);">+<?php  echo $totalConfirmed-$yestotalconfirmed;//计算出较昨日数据  ?></span></p>
        </div>
        ...
        <div class="nowconfirm" onclick="Swal.fire({icon: 'question',title: '疫情数据说明：',html: '<h3>1.数据含义：</h3>' + '<p>“现有确诊数”为当前正在治疗中的确诊人数，此数值会随疫情数据的实时更新而发生变化。</p><br>' + '<h3>2.计算方法：</h3>' + '<p>现有确诊数=累计确诊数-累计死亡数-累计治愈数</p></br>',showConfirmButton: false})">
          ...
          <div class="number" id="number" style="color:<?php if ($nowconfirm > '10') {echo 'RGB(228,74,61)';}elseif($nowconfirm > '0'){echo 'RGB(255,163,82)';}elseif($nowconfirm == '0'){echo 'RGB(0,141,94)';} ?>;">
            <?php echo $nowconfirm; ?></div><p class="added" style="display: block;">较昨日<span style="color: <?php if($yesnowconfirmdata>'10'){echo 'RGB(255,163,82)';}elseif($yesnowconfirmdata>'0'){echo 'RGB(255,163,82)';}elseif($yesnowconfirmdata<='0'){echo 'RGB(0,141,94)';} ?>;"><?php if($yesnowconfirmdata>='0'){echo '+'.$yesnowconfirmdata;}else{echo $yesnowconfirmdata;}//判断数值为正还是负 ?></span></p>
        </div>
      ...
      <hr align=center width=750 color=#8e8e88 SIZE=1></hr>
        ...
        <div class="form"><!--折线图-->
                ...
                var myChart = new Chart(ctx, {
                    type: 'line',
                    data: {
                        labels: [<?php while($row3 = mysqli_fetch_array($query3)){echo "'".$row3['time']."',";} ?>],//输出X轴标题文字(时间)
                        datasets: [{
                            label: '现有确诊',
                            data: [<?php while($row8 = mysqli_fetch_array($query8)){echo $row8['nowconfirm'].",";} ?>],//输出点(数据)
    ...
    <div id="return-top" class="top_e"><!-- 返回最顶按钮(样式看不见emmm)-->
    ...
    <footer>
      <div class="footer"><!--最尾-来一个COPYRIGHT,因为网站摆放在国内服务器,所以还是规规矩矩摆上备案号(雾)-->
